...
- 可选整数倍上采样：通过配置 `upsample_factor` 参数，可在延迟计算前对信号进行上采样，提升延迟检测精度（尤其适合短时延场景），但会增加计算量。
...
- `upsample_factor`：延迟检测时的整数上采样倍数，默认为 1（不进行上采样）。设置为大于 1 的整数（如 4）可提高延迟计算精度，但会增加计算开销。
